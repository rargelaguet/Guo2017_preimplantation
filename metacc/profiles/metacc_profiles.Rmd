...
```{r}
to.plot <- data[,.(ratio=mean(.SD[abs(dist)==0,rate])/mean(.SD[abs(dist)>1800,rate])), by=c("sample","context")] %>%
...
pdf(sprintf("%s/ratios/ratio_met.pdf",io$outdir), width=8.5, height=5)
print(p)
dev.off()
```
...
pdf(sprintf("%s/ratios/ratio_acc.pdf",io$outdir), width=8.5, height=5)
print(p)
dev.off()
```
