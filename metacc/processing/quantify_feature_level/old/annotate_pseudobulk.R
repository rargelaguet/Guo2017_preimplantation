...
if (grepl("ricard",Sys.info()['nodename'])) {
  ...
  # GC
  if (opts$context == "GC") {
    io$in.folder <- paste0(io$basedir,"/acc/gpc_level/pseudobulk")
    io$out.folder <- paste0(io$basedir,"/acc/feature_level/pseudobulk")
  # CG
  } else if (opts$context=="CG") {
    io$in.folder <- paste0(io$basedir,"/met/cpg_level/pseudobulk")
    io$out.folder <- paste0(io$basedir,"/met/feature_level/pseudobulk")
  }
...
if (opts$context=="CG") {
  ...
  samples_keep <- fread(io$in.sample_metadata, header=T) %>% .[stage!="ESC"] %>% .[,id_acc]
} else{
  stop()
}
...
for (i in 1:length(samples_keep)) {
  ...
  if (all(sprintf("%s_%s.gz",sample,opts$anno) %in% files_processed)) {
    ...
    dat_sample <- fread(cmd=sprintf("zcat < %s/%s.tsv.gz",io$in.folder,sample), sep="\t", verbose=F, showProgress=F, header=T) %>%
