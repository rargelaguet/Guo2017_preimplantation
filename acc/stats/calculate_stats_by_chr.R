...
sample_metadata <- sample_metadata %>% .[!is.na(id_acc)]
...
stats <- data.table(expand.grid(as.character(sample_metadata$id_acc),opts$chr)) %>% 
  setnames(c("id_acc","chr")) %>%
  .[,c("coverage","mean"):=as.numeric(NA)]

for (i in sample_metadata$id_acc) {
  if (file.exists(sprintf("%s/%s.tsv.gz",io$acc_data_raw,i))) {
    ...
    for (j in opts$chr) {
      data_j <- data[chr==j]
      # stats[id_acc==i & chr==j, c("nreads","coverage","mean"):=list(sum(data_j$met_reads+data_j$nonmet_reads), nrow(data_j),round(mean(data_j$rate)*100,2))]  
      stats[id_acc==i & chr==j, c("coverage","mean"):=list(nrow(data_j),round(mean(data_j$rate)*100,2))]
    }
