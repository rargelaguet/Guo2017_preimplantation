---
title: "Sex determination per cell based on CG coverage along chrY"
---

```{r load_modules, echo=FALSE, include=FALSE}
library(ggpubr)
```

Define settings
```{r define_opts, echo=FALSE}
if (grepl("ricard",Sys.info()['nodename'])) {
  source("/Users/ricard/Guo2017_preimplantation/settings.R")
} else {
  stop("Computer not recognised")
}
io$outdir <- paste0(io$basedir,"/met/sex")

# Define stages
opts$stages <- c(
  "Zygote",
  "2-cell",
  "4-cell",
  "8-cell",
  "Morula",
  "ICM",
  "TE"
)

# Define which chromosomes to look at
opts$chr <- c("X","Y","1") %>% paste0("chr",.)
```

Update sample metadata
```{r}
sample_metadata <- sample_metadata %>% 
  .[!is.na(id_met) & stage%in%opts$stages]
```

# Load data

Load DNA methylation statistics per chromosome
```{r}
stats <- fread(io$met.stats_per_chr) %>%
  .[,chr:=paste0("chr",chr)] %>%
  .[chr%in%opts$chr] %>%
  merge(sample_metadata, by="id_met") %>%
  .[,.(coverage=sum(coverage)), by=c("id_met","stage","chr")]
```

# Plot

```{r}
to.plot <- stats %>% 
  data.table::dcast(id_met+stage~chr, value.var="coverage") %>%
  .[,ratio:=chrY/chr1]
```

Define sex
```{r}
opts$female <- 0.001      # Less than this is female
opts$male <- 0.00101    # More than this is male
# to.plot[ratio>op]

to.plot$sex <- cut(to.plot$ratio, 
  breaks = c(-0.01,opts$female, opts$male,Inf),
  labels = c("Female","Unassigned","Male")
)
```


Plot ratio of coverage between Y chr and X chr
```{r}
p <- ggbarplot(to.plot, x="id_met", y="ratio", fill="sex", sort.val="asc") +
  facet_grid(~stage, scales="free_x", space = "free_x") +
  labs(x="Embryo", y="ratio chrY/chr1 coverage") +
  geom_hline(yintercept=opts$female, linetype="dashed") +
  theme(
    legend.title = element_blank(),
    axis.ticks.x = element_blank(),
    axis.text.x = element_blank()
  )

# pdf(paste0(io$outdir,"/sex_assignment_met.pdf"), width=9, height=5, useDingbats = F)
print(p)
# dev.off()
```

Save results
```{r}
# fwrite(to.plot, paste0(io$outdir,"/sex_assignment_met.txt.gz"))
```

Add to sample metadata
```{r}
to.save <- to.plot[,c("id_met","stage","sex")] %>%
  setnames("sex","sex_per_cell")

sample_metadata <- fread(io$metadata) %>% 
  merge(to.save, by=c("id_met","stage"), all.x=T)

table(sample_metadata$sex)
table(sample_metadata[is.na(sex),stage])

# View(sample_metadata[is.na(sex)])
# sample_metadata[embryo=="693-D6 ICM Embryo 5"]
```

```{r}
# fwrite(sample_metadata, io$metadata, sep="\t", na="NA", quote=F)
```

